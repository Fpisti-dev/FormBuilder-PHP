...
function newPage() {
    ...
    //"last_page" link id 
    $('#link_page_last').before('<li id="link_page_' + pageNum + '" class="list-group-item ui-sortable" style="text-align: center;" data-div="page_'
...
function LoadFormByName(sFormName) {
    ...
    $.ajax({
        ...
        success: function (result) {
			...
			// If form name incorrect redirect to home page
			if (res === undefined) { 			
				...
				if (res.PageHtml) {
					...
					$('#dPages').empty().html(res.PageHtml);
					$('#dDesigner').empty().html(res.DesignerHtml);
					...
					$('#hfStyling').val(res.Styling);
					$('#hfScripting').val(res.Scripting);
					$('#hfResourcesStyle').val(res.ResourcesStyle);
					$('#hfResourcesScript').val(res.ResourcesScript);
				}    
				else {
					alert("Unsaved form !!!");
				}
...
function btnSaveForm_Click() {
    ...
    $.ajax({
        method: "POST",
        url: "~/../Handlers/SaveFormByName.php",
        dataType: "json",
        async: true,
        data: {
            FormName: id,
...
function btnPreviewForm_Click() {
    ...
    var fID = $.urlParam('fID'); //without spaces
    var id = decodeURIComponent($.urlParam('fID')); //with spaces
    window.open('~/../Preview.php?fID=' + fID, '_blank');
};
...
function btnLaunchForm_Click() {
    ...
    var fID = $.urlParam('fID'); //without spaces
    var id = decodeURIComponent($.urlParam('fID')); //with spaces
    window.open('~/../Form.php?fID=' + fID, '_blank');
};
